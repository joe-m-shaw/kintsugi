...
Different rows in numeric columns (coverage, pct_mapped_reads, score, ccne1_cn and rad51b_cn) have different numbers of decimal places.
...
Harmonise these to consistently have `r decimal_places` decimal places each.
...
gi_csv_bind_correct_decimals <- gi_csv_bind |> 
  mutate(coverage = round(coverage, digits = decimal_places),
         pct_mapped_reads = round(pct_mapped_reads, digits = decimal_places),
         score = round(score, digits = decimal_places),
         ccne1_cn = round(ccne1_cn, digits = decimal_places),
         rad51b_cn = round(rad51b_cn, digits = decimal_places))

stopifnot(anyNA.data.frame(gi_csv_bind_correct_decimals |> 
                   select(coverage, pct_mapped_reads,
                          score, ccne1_cn, rad51b_cn)) == FALSE)

gi_csv_bind_pre_oct2025 <- gi_csv_bind_correct_decimals |> 
...
Get information (iGene R number, iGene S number, first name, surname, NHS number, pathology block identifier and date of birth) from DNA Database for each DNA laboratory number.

This includes calculating the age at SeqOne GI testing for each patient.
...
gi_csv_collated_patient_info <- gi_csv_bind_pre_oct2025 |> 
  ...
  mutate(age_at_testing = floor(x = interval(dob, date) / years())) |> 
  relocate(labno, i_gene_r_no, i_gene_s_no,
           surname, firstname, nhsno, dob, date, 
           age_at_testing, pathno, comments) |> 
...
```{r}
#| label: check-age-calculation

samples_with_dobs <- gi_csv_collated_patient_info |> 
  filter(!is.na(dob))

stopifnot(max(samples_with_dobs$age_at_testing) < 100)
stopifnot(min(samples_with_dobs$age_at_testing) > 25)

ggplot(samples_with_dobs, aes(x = age_at_testing, y = )) +
  geom_histogram(binwidth = 1) +
  theme_bw() +
  labs(x = "Age at SeqOne GI testing (years)",
       y = "Number of patients")

```

The ages of patients in this cohort ranges from
`r min(samples_with_dobs$age_at_testing)` to
`r max(samples_with_dobs$age_at_testing)`
(median `r median(samples_with_dobs$age_at_testing)`).
...
stopifnot(anyNA(x = gi_csv_cleaned_orpp$dob) == FALSE)

stopifnot(anyNA(x = gi_csv_cleaned_orpp$age_at_testing) == FALSE)
...
final_dataset_patient_ids_removed <- final_dataset_patient_ids |> 
  select(-c(gi_labno, full_name, rno.x, nhsno,  dob, date, tvar_data_source))
