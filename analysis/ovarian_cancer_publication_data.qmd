...
Dr Morgan originally also requested data on germline testing for *BRCA1* and *BRCA2*. 
Germline testing data is now no longer required for this publication, but the code for collating germline data has been left to allow easier collation for the next publication.

# Methods

Preparing this data requires multiple steps and uses the `kintsugi` package. For each step I have included explanatory text alongside the R code.

Here is an overview of the approach I have taken:

- Collate all GI data from individual csv files saved on the S drive
...
This is exlained in more detail in the `names.qmd` vignette in the `kintsugi` package.
...
Collate GI results by reading the first line of each .csv file. Two files are excluded from this process as they were from a failed pipeline run and have NA values for several variables. These samples were successfully re-analysed.
...
gi_ws_collated_pre_oct2025 <- gi_ws_collated |> 
  filter(date < "2025-10-20 00:00:00")

gi_csv_duplicates <- gi_ws_collated_pre_oct2025 |> 
...
gi_labnos <- unique(gi_ws_collated_pre_oct2025$labno)
...
gi_csv_collated_patient_info <- gi_ws_collated_pre_oct2025 |> 
...
There are `r nrow(patients_without_nhsno)` samples from patients which do not have an NHS number.
These are removed from the dataset because it is not possible to link them to other test results.

There are `r nrow(non_patient_samples)` samples from EQA schemes or synthetic controls.
...
There are some panels which a minority of patients receive, such as "OvarianEXT_PS v2.b" and "M119_PAEDEXT_PS".
Results from these panels are not included in this analysis.
...
iGene germline variant data on iGene ranges from `r format.Date(x = min(glvar_igene_no_na$test_order_date), format = "%d %B %Y")` to `r format.Date(x = max(glvar_igene_no_na$test_order_date), format = "%d %B %Y")`.

iGene tumour variant data on iGene ranges from `r format.Date(x = min(tvar_igene_no_na$test_order_date), format = "%d %B %Y")` to `r format.Date(x = max(tvar_igene_no_na$test_order_date), format = "%d %B %Y")`.
...
Annotate variant classifications to DNA Database germline results.
To do this I had to manually check each DNA Database report to check what the classification of each variant was.
...
This splits the "genotype" column into separate columns.
One sample has a pathogenic *BRAF* and a pathogenic *TP53* variant in the genotype column.
As neither as *BRCA* variants I have not created a separate column for the second variant.
...
Join tvar and glvar data by NHS number.
...
Restrict the dataset to results between 1st December 2023 and 20th October 2025.
The date used for this step is the date of the SeqOne genomic instability analysis.

```{r}
#| label: restrict-date-range

gi_tvar_joined_by_nhsno_date_restricted <- gi_tvar_joined_by_nhsno |> 
  filter(date > "2023-12-01 00:00:00" &
           date < "2025-10-20 00:00:00") |> 
  rename(tvar_data_source = data_source)

# Check for duplicates
stopifnot(anyDuplicated(gi_tvar_joined_by_nhsno_date_restricted$nhsno) == 0)

# Check for EQA samples
stopifnot(length(grep(pattern = "EQA|Sera", 
                      x = gi_tvar_joined_by_nhsno_date_restricted$full_name,
                      ignore.case = TRUE,
                      value = TRUE)) == 0)
...
gi_tvar_joined_by_nhsno_select <- gi_tvar_joined_by_nhsno_date_restricted |> 
  select(
    gi_labno,
    rno.x,
    full_name,
...
data_for_publication <- gi_tvar_joined_by_nhsno_brca_only

write_csv(gi_tvar_joined_by_nhsno_brca_only,
          paste0(joined_folder, "data_for_publication.csv"))

```

I have performed spot checks on the following samples to check that the results match those in DNA Database and iGene.

24069071, 25023434, 25012444, 25045737, 24013968, 
25003285, 24012711, 25029049, 25038927, 24040237,
24014817, 25002153, 24056470, 25001726, 23060393

# Conclusion

`r nrow(data_for_publication)` results have been identified which meet the data request criteria.
...
`r format.Date(x = min(data_for_publication$date), format = "%d %B %Y")` and
`r format.Date(x = max(data_for_publication$date), format = "%d %B %Y")`.
