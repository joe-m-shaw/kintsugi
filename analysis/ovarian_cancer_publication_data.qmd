...
Restrict the dataset to results between 1st December 2023 and 20th October 2025. The date used for this step is the date of the SeqOne genomic instability analysis.
...
-   The data was collected between `r format.Date(x = min(data_for_publication$date), format = "%d %B %Y")` and `r format.Date(x = max(data_for_publication$date), format = "%d %B %Y")`.
