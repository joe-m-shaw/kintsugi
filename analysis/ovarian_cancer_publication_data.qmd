...
The naming convention for ovarian cancer testing terms is defined in the `names.qmd` vignette in `kintsugi`.
...
Now do the same thing for tumour *BRCA* results. 
...
There are 2 important differences between variant data in DNA Database and in iGene:
...
Now the datasets can be joined together for analysis. 
There are different ways in which the data can be joined:
